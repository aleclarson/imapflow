...
import { compiler } from './handler/imap-handler'
import { createHash } from 'crypto'
import { JPDecoder } from './jp-decoder'
...
export function encodePath(connection, path) {
  path = (path || '').toString()
  if (
    !connection.enabled.has('UTF8=ACCEPT') &&
    /[&\x00-\x08\x0b-\x0c\x0e-\x1f\u0080-\uffff]/.test(path)
  ) {
    try {
...
export function normalizePath(connection, path, skipNamespace?) {
  if (Array.isArray(path)) {
    path = path.join(
      (connection.namespace && connection.namespace.delimiter) || ''
    )
  }
  ...
  if (
    ...
    connection.namespace.prefix &&
    path.indexOf(connection.namespace.prefix) !== 0
  ) {
...
export function getFolderTree(folders): any {
  let tree = {
    root: true,
    folders: [] as any[],
  }
  ...
  let getTreeNode = parents => {
    ...
    for (let parent of parents) {
      let cur =
        node.folders && node.folders.find(folder => folder.name === parent)
      if (cur) {
        node = cur
      } else {
        // not yet set
  ...
  for (let folder of folders) {
    ...
    let existing =
      parent.folders &&
      parent.folders.find(existing => existing.name === folder.name)
    if (existing) {
      // update values
...
export function getColorFlags(color) {
  const colorCode = color
    ? FLAG_COLORS.indexOf((color || '').toString().toLowerCase().trim())
    : null
  ...
  let result = {
    add: colorCode ? ['\\Flagged'] : [],
    remove: colorCode ? [] : ['\\Flagged'],
  }
...
export async function formatMessageResponse(untagged, mailbox): Promise<any> {
  let map: any = {}
  ...
  for (let i = 0, len = attributes.length; i < len; i++) {
    ...
    let getArray = attribute => {
      if (Array.isArray(attribute)) {
        return attribute
          .map(entry =>
            entry && typeof entry.value === 'string' ? entry.value : false
          )
          .filter(entry => entry)
      }
    }
    ...
    switch (key) {
      ...
      case 'modseq':
        ...
        if (
          map.modseq &&
          (!mailbox.highestModseq || mailbox.highestModseq < map.modseq)
        ) {
  ...
  if (map.emailId || map.uid) {
    ...
    map.id =
      ...
      createHash('md5')
        .update(
          [path, mailbox.uidValidity.toString(), map.uid.toString()].join(':')
        )
        .digest('hex')
  }
...
export function parseEnvelope(entry) {
  ...
  let processAddresses = function (list) {
      return []
        ...
        .map(addr => {
          let address =
            (getStrValue(addr[2]) || '') + '@' + (getStrValue(addr[3]) || '')
          if (address === '@') {
            address = ''
          }
          return {
            name: processName(libmime.decodeWords(getStrValue(addr[0]))),
            address,
          }
        })
  ...
  let walk = (node, path) => {
    ...
    if (i < node.length - 1) {
      if (node[i]) {
        curNode.language = []
          .concat(node[i] || [])
          .map(val => ((val && val.value) || '').toString().toLowerCase())
      }
      i++
    }
...
export function formatDateTime(value) {
  ...
  let dateStr = formatDate(value).replace(/^0/, '') //starts with date-day-fixed with leading 0 replaced by SP
...
export function canUseFlag(mailbox, flag) {
  return (
    ...
    mailbox.permanentFlags.has('\\*') ||
    mailbox.permanentFlags.has(flag)
  )
}
