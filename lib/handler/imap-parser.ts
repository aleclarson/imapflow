...
import { ParserInstance } from './parser-instance'

export = async (command, options) => {
  options = options || {}
  ...
  try {
    ...
    if (
      ['UID', 'AUTHENTICATE'].indexOf((response.command || '').toUpperCase()) >=
      0
    ) {
      await parser.getSpace()
      response.command +=
        ' ' + (await parser.getElement(imapFormalSyntax.command()))
    }
    ...
    if (
      err.code === 'ParserErrorExchange' &&
      err.parserContext &&
      err.parserContext.value
    ) {
