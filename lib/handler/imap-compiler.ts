 ...
 */
export = async (response, options: any = {}) => {
  let { asArray, isLogging, literalPlus, literalMinus } = options
  const respParts = []

  let resp = []
    .concat(formatRespEntry(response.tag, true) || [])
    .concat(response.command ? formatRespEntry(' ' + response.command) : [])
  let val
  let lastType
  ...
  let walk = async (node, options) => {
    ...
    let lastRespByte =
      (lastRespEntry &&
        lastRespEntry.length &&
        lastRespEntry[lastRespEntry.length - 1]) ||
      ''
    ...
    switch (node.type.toUpperCase()) {
      case 'LITERAL':
        if (isLogging) {
          resp.push(
            formatRespEntry('"(* ' + node.value.length + 'B literal *)"')
          )
        } else {
          let literalLength = !node.value ? 0 : Math.max(node.value.length, 0)

          let canAppend =
            !asArray || literalPlus || (literalMinus && literalLength <= 4096)
          let usePlus = canAppend && (literalMinus || literalPlus)

          resp.push(
            formatRespEntry(
              `${node.isLiteral8 ? '~' : ''}{${literalLength}${
                usePlus ? '+' : ''
              }}\r\n`
            )
          )
      ...
      case 'STRING':
        if (isLogging && node.value.length > 100) {
          resp.push(
            formatRespEntry('"(* ' + node.value.length + 'B string *)"')
          )
        } else {
          resp.push(
            formatRespEntry(JSON.stringify((node.value || '').toString()))
          )
        }
        break
  ...
  if (response.attributes) {
    let attributes = Array.isArray(response.attributes)
      ? response.attributes
