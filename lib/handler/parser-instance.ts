...
export class ParserInstance {
  ...
  async getElement(syntax) {
    ...
    if (this.remainder.match(/^\s/)) {
      let error = new Error(
        `Unexpected whitespace at position ${this.pos} [E1]`
      )
    ...
    if ((match = this.remainder.match(/^\s*[^\s]+(?=\s|$)/))) {
      ...
      if ((errPos = imapFormalSyntax.verify(element, syntax)) >= 0) {
        ...
        let error = new Error(
          `Unexpected char at position ${
            this.pos + errPos
          } [E2: ${JSON.stringify(element.charAt(errPos))}]`
        )
      ...
      let error = new Error(
        `Unexpected end of input at position ${this.pos} [E3]`
      )
  ...
  async getSpace() {
    if (!this.remainder.length) {
      let error = new Error(
        `Unexpected end of input at position ${this.pos} [E4]`
      )
    ...
    if (
      imapFormalSyntax.verify(
        this.remainder.charAt(0),
        imapFormalSyntax.SP()
      ) >= 0
    ) {
      let error = new Error(
        `Unexpected char at position ${this.pos} [E5: ${JSON.stringify(
          this.remainder.charAt(0)
        )}]`
      )
  ...
  async getAttributes() {
    if (!this.remainder.length) {
      let error = new Error(
        `Unexpected end of input at position ${this.pos} [E6]`
      )
    ...
    if (this.remainder.match(/^\s/)) {
      let error = new Error(
        `Unexpected whitespace at position ${this.pos} [E7]`
      )
