...
import imapFormalSyntax = require('./imap-formal-syntax')
    ...
    for (i = 0, len = this.str.length; i < len; i++) {
      ...
      switch (this.state) {
          ...
          switch (chr) {
            ...
            case ')':
              if (this.currentNode.type !== 'LIST') {
                let error = new Error(
                  `Unexpected list terminator ) at position ${
                    this.pos + i
                  } [E10]`
                )
            ...
            case ']':
              if (this.currentNode.type !== 'SECTION') {
                let error = new Error(
                  `Unexpected section terminator ] at position ${
                    this.pos + i
                  } [E11]`
                )
            ...
            case '~': {
              ...
              if (nextChr !== '{') {
                ...
                let error = new Error(
                  `Unexpected literal8 marker at position ${this.pos + i} [E12]`
                )
            ...
            case '{':
              ...
              this.currentNode.literalType =
                this.expectedLiteralType || 'literal'
            ...
            case '[':
              ...
              if (
                ['OK', 'NO', 'BAD', 'BYE', 'PREAUTH'].includes(
                  this.parent.command.toUpperCase()
                ) &&
                ...
                if (this.str.substr(i + 1, 9).toUpperCase() === 'REFERRAL ') {
                  ...
                  this.currentNode.value = this.str.substring(
        ...
        case STATE_ATOM:
          ...
          if (
            (chr === ',' || chr === ':') &&
            RE_DIGITS.test(this.currentNode.value)
          ) {
            this.currentNode.type = 'SEQUENCE'
            this.currentNode.isClosed = true
            this.state = STATE_SEQUENCE
          ...
          if (chr === '[') {
            ...
            if (
              ['BODY', 'BODY.PEEK', 'BINARY', 'BINARY.PEEK'].indexOf(
                this.currentNode.value.toUpperCase()
              ) < 0
            ) {
              let error = new Error(
                `Unexpected section start char [ at position ${
                  this.pos + i
                } [E14]`
              )
          ...
          if (chr === '<') {
            let error = new Error(
              `Unexpected start of partial at position ${this.pos + i} [E15]`
            )
          ...
          if (
            ...
            (!this.parent ||
              !this.parent.command ||
              !['NO', 'BAD', 'OK'].includes(this.parent.command))
          ) {
            let error = new Error(
              `Unexpected char at position ${
                this.pos + i
              } [E16: ${JSON.stringify(chr)}]`
            )
        ...
        case STATE_STRING:
          ...
          if (chr === '\\') {
            ...
            if (i >= len) {
              let error = new Error(
                `Unexpected end of input at position ${this.pos + i} [E18]`
              )
        ...
        case STATE_PARTIAL:
          if (chr === '>') {
            if (this.currentNode.value.at(-1) === '.') {
              let error = new Error(
                `Unexpected end of partial at position ${this.pos + 1} [E19]`
              )
          ...
          if (
            chr === '.' &&
            (!this.currentNode.value.length ||
              this.currentNode.value.match(/\./))
          ) {
            let error = new Error(
              `Unexpected partial separator . at position ${this.pos + 1} [E20]`
            )
          ...
          if (this.currentNode.value.match(/^0$|\.0$/) && chr !== '.') {
            let error = new Error(
              `Invalid partial at position ${
                this.pos + i
              } [E22: ${JSON.stringify(chr)}]`
            )
        ...
        case STATE_LITERAL:
          if (this.currentNode.started) {
            ...
            this.currentNode.chBuffer[this.currentNode.chPos++] =
              chr.charCodeAt(0)
            ...
            if (this.currentNode.chPos >= this.currentNode.literalLength) {
              ...
              this.currentNode.value =
                this.currentNode.chBuffer.toString('binary')
          ...
          if (chr === '}') {
            if (!('literalLength' in this.currentNode)) {
              let error = new Error(
                `Unexpected literal prefix end char } at position ${
                  this.pos + i
                } [E23]`
              )
            ...
            this.currentNode.literalLength = Number(
              this.currentNode.literalLength
            )
            ...
            if (!this.currentNode.literalLength) {
              ...
              this.currentNode.endPos =
                this.pos + i + this.currentNode.value.length
              ...
              this.currentNode.chBuffer = Buffer.alloc(
                this.currentNode.literalLength
              )
          ...
          if (this.currentNode.literalLength === '0') {
            let error = new Error(
              `Invalid literal at position ${this.pos + i} [E26]`
            )
          ...
          this.currentNode.literalLength =
            (this.currentNode.literalLength || '') + chr
          break
        ...
        case STATE_SEQUENCE:
          ...
          if (chr === ' ') {
            if (
              !RE_SINGLE_DIGIT.test(this.currentNode.value.at(-1)) &&
              this.currentNode.value.at(-1) !== '*'
            ) {
              let error = new Error(
                `Unexpected whitespace at position ${this.pos + i} [E27]`
              )
            ...
            if (
              this.currentNode.value !== '*' &&
              this.currentNode.value.at(-1) === '*' &&
              this.currentNode.value.at(-2) !== ':'
            ) {
              let error = new Error(
                `Unexpected whitespace at position ${this.pos + i} [E28]`
              )
          ...
          if (chr === ':') {
            if (
              ...
              let error = new Error(
                `Unexpected range separator : at position ${this.pos + i} [E29]`
              )
            ...
            if ([',', ':'].indexOf(this.currentNode.value.at(-1)) < 0) {
              let error = new Error(
                `Unexpected range wildcard at position ${this.pos + i} [E30]`
              )
            ...
            if (
              ...
              let error = new Error(
                `Unexpected sequence separator , at position ${
                  this.pos + i
                } [E31]`
              )
          ...
          if (
            RE_SINGLE_DIGIT.test(chr) &&
            this.currentNode.value.at(-1) === '*'
          ) {
            let error = new Error(
              `Unexpected number at position ${
                this.pos + i
              } [E34: ${JSON.stringify(chr)}]`
            )
