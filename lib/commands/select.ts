...
// Selects a mailbox
export const SELECT = async (connection, path, options) => {
  if (
  ...
  let folderListData = connection.folders.has(path)
    ? connection.folders.get(path)
    : false
  ...
  try {
      ...
      selectCommand.arguments,
      {
        untagged: {
          OK: async untagged => {
            ...
            let section =
              !untagged.attributes[0].value && untagged.attributes[0].section
            if (
              section &&
              section.length > 1 &&
              section[0].type === 'ATOM' &&
              ...
              if (typeof section[1].value === 'string') {
                ...
                value = section[1]
                  .map(entry =>
                    typeof entry.value === 'string' ? entry.value : false
                  )
                  .filter(entry => entry)
              }
          ...
          FLAGS: async untagged => {
            if (
              !untagged.attributes ||
              (!untagged.attributes.length &&
                Array.isArray(untagged.attributes[0]))
            ) {
              return
            }
            let flags = untagged.attributes[0]
              .map(flag =>
                typeof flag.value === 'string' ? flag.value : false
              )
              .filter(flag => flag)
    ...
    let section =
      !response.response.attributes[0].value &&
