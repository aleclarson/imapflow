...
export const FETCH = async (connection, range, query, options) => {
  ...
  const commandKey =
    connection.capabilities.has('BINARY') &&
    options.binary &&
    !connection.disableBinary
      ? 'BINARY'
      : 'BODY'
    ...
    try {
      ...
      if (query.source) {
        ...
        if (
          ...
          (query.source.start || query.source.maxLength)
        ) {
      ...
      if (query.headers) {
        if (Array.isArray(query.headers)) {
          setBodyPeek([
            ...
            query.headers.map(header => ({ type: 'ATOM', value: header })),
          ])
        } else {
      ...
      if (
        ...
        connection.enabled.has('CONDSTORE') &&
        !mailbox.noModseq
      ) {
