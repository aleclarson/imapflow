...
export const MOVE = async (connection, range, destination, options) => {
  ...
  if (!connection.capabilities.has('MOVE')) {
    ...
    await connection.messageDelete(
      range,
      ...
      case 'COPYUID':
        {
          let uidValidity =
            section[1] &&
            typeof section[1].value === 'string' &&
            !isNaN(section[1].value)
              ? BigInt(section[1].value)
              : false
          if (uidValidity) {
          ...
          let sourceUids =
            section[2] && typeof section[2].value === 'string'
              ? expandRange(section[2].value)
              : false
          let destinationUids =
            section[3] && typeof section[3].value === 'string'
              ? expandRange(section[3].value)
              : false
          if (
            sourceUids &&
            destinationUids &&
            sourceUids.length === destinationUids.length
          ) {
            map.uidMap = new Map(
              sourceUids.map((uid, i) => [uid, destinationUids[i]])
            )
          }
        }
        break
