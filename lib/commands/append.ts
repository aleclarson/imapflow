...
export const APPEND = async (
  ...
  if (connection.capabilities.has('APPENDLIMIT')) {
    ...
    if (typeof appendLimit === 'number' && appendLimit < content.length) {
      let err = new Error(
        'Message content too big for APPENDLIMIT=' + appendLimit
      )
  ...
  let expectExists = comparePaths(
    connection,
    connection.mailbox.path,
    destination
  )
  ...
  try {
    ...
    if (section && section.length) {
      let responseCode =
        section[0] && typeof section[0].value === 'string'
          ? section[0].value
          : ''
        ...
        case 'APPENDUID':
          {
            ...
            let uid =
              section[2] &&
              typeof section[2].value === 'string' &&
              !isNaN(section[2].value)
                ? Number(section[2].value)
                : false
            if (uidValidity) {
