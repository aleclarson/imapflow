...
export const LIST = async (connection, reference, mailbox, options) => {
  ...
  let listCommand =
    connection.capabilities.has('XLIST') &&
    !connection.capabilities.has('SPECIAL-USE')
      ? 'XLIST'
      : 'LIST'
  ...
  try {
    ...
    if (
      listCommand === 'LIST' &&
      connection.capabilities.has('LIST-STATUS') &&
      statusQueryAttributes.length
    ) {
    ...
    let specialUseHints = {}
    if (
      options.specialUseHints &&
      typeof options.specialUseHints === 'object'
    ) {
      for (let type of Object.keys(options.specialUseHints)) {
        if (
          ...
          specialUseHints[
            normalizePath(connection, options.specialUseHints[type])
      ...
      response = await connection.exec(listCommand, cmdArgs, {
        untagged: {
          [listCommand]: async untagged => {
            ...
            let entry = {
              path: normalizePath(
                ...
                decodePath(
                  connection,
                  (untagged.attributes[2] && untagged.attributes[2].value) || ''
                )
              ),
              pathAsListed:
                (untagged.attributes[2] && untagged.attributes[2].value) || '',
            ...
            entry.parentPath =
              entry.delimiter && entry.path
                ? entry.path.substr(0, entry.path.lastIndexOf(entry.delimiter))
                : ''
          ...
          STATUS: async untagged => {
            let statusPath = normalizePath(
              connection,
              decodePath(
                connection,
                (untagged.attributes[0] && untagged.attributes[0].value) || ''
              )
            )
            let statusList =
              untagged.attributes && Array.isArray(untagged.attributes[1])
                ? untagged.attributes[1]
                : false
      ...
      for (let entry of entries) {
        if (
          ...
          if (statusMap.has(entry.path)) {
            ...
            // run STATUS command
            try {
              entry.status = await connection.run(
                'STATUS',
                entry.path,
                options.statusQuery
              )
    ...
    return entries.sort((a, b) => {
      ...
      if (a.specialUse && b.specialUse) {
        return (
          FLAG_SORT_ORDER.indexOf(a.specialUse) -
          FLAG_SORT_ORDER.indexOf(b.specialUse)
        )
      }
